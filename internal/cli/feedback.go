...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".espn-sports-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ESPN_SPORTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ESPN_SPORTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "espn-sports-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.espn-sports-pp-cli/feedback.jsonl.
When ` + "`ESPN_SPORTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ESPN_SPORTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  espn-sports-pp-cli feedback list
  espn-sports-pp-cli feedback list --limit 5
  espn-sports-pp-cli feedback list --json`,
