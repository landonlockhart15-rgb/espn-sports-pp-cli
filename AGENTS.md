# Espn Sports Printed CLI Agent Guide

This directory is a generated `espn-sports-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
espn-sports-pp-cli doctor --json
espn-sports-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
espn-sports-pp-cli which "<capability>" --json
espn-sports-pp-cli <command> --help
```
...
```bash
espn-sports-pp-cli <command> --agent
```
...
```bash
espn-sports-pp-cli <command> --help
espn-sports-pp-cli <command> --dry-run --agent
```
